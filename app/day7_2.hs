...
isPossiblyTrue t ops s = isPossiblyTrue t (tail ops) (s + head ops) || isPossiblyTrue t (tail ops) (s * head ops) || isPossiblyTrue t (tail ops) (concatOp s (head ops))  -- try +, *, or concat with next operand
